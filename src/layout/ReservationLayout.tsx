...
type ReservationLayoutType = {
  children: React.ReactNode;
  page: string;
};

const ReservationLayout: React.FC<ReservationLayoutType> = ({ children, page }) => {
  return (
    <>
      <Banner>
        <div className="title">{'비대면 면회 및\n영상 통화를 예약해보세요.'}</div>
      </Banner>
      <Container>
        <SideMenu>
          <MenuItem select={page === 'reservation'}>
            <StyledLink to="/reservation">예약 하기</StyledLink>
          </MenuItem>
          <MenuItem select={page === 'reservationList'}>
